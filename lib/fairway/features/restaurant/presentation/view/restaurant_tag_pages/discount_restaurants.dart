...
class DiscountRestaurants extends StatelessWidget {
  const DiscountRestaurants({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<RestaurantCubit, RestaurantState>(
      builder: (context, state) {
        final restaurants = state.discountRestaurants.data?.restaurants ?? [];

        if (state.discountRestaurants.isLoading &&
            state.discountCurrentPage == 1) {
          return Column(
        ...
        if (state.discountRestaurants.isFailure &&
            state.discountCurrentPage == 1) {
          return RetryWidget(
            onRetry: () =>
                context.read<RestaurantCubit>().getDiscountRestaurants(
                      page: state.drinksCurrentPage,
                      sortBy: state.selectedSortOption,
                    ),
            message: state.discountRestaurants.errorMessage ??
                'Failed to load discount restaurants',
          );
        }
        ...
        return PaginatedListView<RestaurantModel>(
          ...
          onLoadMore: () =>
              context.read<RestaurantCubit>().getDiscountRestaurants(
                    page: state.discountCurrentPage + 1,
                    sortBy: state.selectedSortOption,
                    showLoading: false,
                  ),
          isLoadingMore: state.discountRestaurants.isPageLoading,
          hasMore: state.drinksCurrentPage <
              (state.discountRestaurants.data?.totalPages ?? 0),
        );
      },
    );
  }
